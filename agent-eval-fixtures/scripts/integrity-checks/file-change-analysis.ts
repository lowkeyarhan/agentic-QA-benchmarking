...
function analyzeChanges(changes: FileChange[]): AnalysisResult {
  ...
  const testLOC = testFiles.reduce((sum, f) => sum + f.additions + f.deletions, 0);
  const implLOC = implFiles.reduce((sum, f) => sum + f.additions + f.deletions, 0);
  ...
  if (testFiles.length > 0 && implFiles.length === 0) {
    suspicious = true;
    severity = 'critical';
    reason = 'Only test files modified - no implementation changes. This suggests assertion weakening or test gaming.';
  } else if (ratio > 3) {
    suspicious = true;
    severity = 'warning';
...
function main() {
  ...
  if (analysis.suspicious) {
    ...
    console.log('   Implementation changes appropriately accompany test changes');
    process.exit(0);
  }
}
