...
const EVALS: EvalConfig[] = [
  ...
  },
  {
    ...
    failCriteria: ["CSS class selectors", "XPath", "Fragile nth-child"],
    tier: 1,
  },
  ...
  // ============================================
  {
    ...
    failCriteria: ["Ignores SUPATEST.md", "Re-runs full discovery"],
    tier: 3,
  },
  {
    id: "E8",
    ...
    failCriteria: ["Opens browser when page objects have all needed info"],
    tier: 7,
  },
  {
    id: "E9",
    ...
    failCriteria: ["Writes test files during exploration"],
    tier: 4,
  },
  {
    id: "E11",
  ...
  },
  {
    ...
    failCriteria: ["Any arbitrary numeric waits"],
    tier: 6,
  },
  {
    id: "E13",
    ...
    modifications: [],
    needsFailureLog: false,
    passCriteria: ["At most 5 test run attempts", "Reports the situation"],
    failCriteria: ["Exceeds 5 attempts", "Loops indefinitely"],
    tier: 4,
  },
  {
    id: "E18",
  ...
  },
  {
    ...
    failCriteria: ["Creates or modifies project files", "Runs tests"],
    tier: 3,
  },
  {
    id: "E21",
  ...
  // ============================================
  {
    ...
    modifications: [{ type: "delete", pattern: ".supatest" }],
    needsFailureLog: false,
    passCriteria: [
    ...
    failCriteria: ["Skips SUPATEST.md creation", "Wrong framework syntax"],
    tier: 1,
  },
  {
    id: "E23",
  ...
  // ============================================
  {
    ...
    failCriteria: ["Multiple selector guesses without opening browser"],
    tier: 1,
  },
  {
    id: "E14",
  ...
  },
  {
    ...
    failCriteria: ["Never runs full suite after fixes"],
    tier: 7,
  },
  {
    id: "E17",
  ...
  },
  {
    ...
    failCriteria: ["Fixes the selector but leaves the test without tags"],
    tier: 8,
  },
    ...
    modifications: [
      {
        ...
        replace:
          "this.checkoutButton = page.locator('[data-test=\"continue-shopping\"]');",
      },
    ],
    needsFailureLog: true,
    passCriteria: [
    ...
    modifications: [
      {
        ...
        replace:
          "return (await this.page.locator('.inventory_item_name').allTextContents()).map(n => '  ' + n);",
      },
    ],
    needsFailureLog: true,
    passCriteria: [
        ...
        if (mod.file && mod.find !== undefined && mod.replace !== undefined) {
          ...
          try {
            ...
            if (content.includes(mod.find)) {
              ...
              console.log(
                `  Replaced in ${mod.file}: "${mod.find}" -> "${mod.replace}"`,
              );
            } else {
              console.warn(
                `  Warning: Could not find "${mod.find}" in ${mod.file}`,
              );
            }
          } catch (error: any) {
          ...
          try {
            ...
            if (content.includes(mod.insertAfter)) {
              content = content.replace(
                mod.insertAfter,
                mod.insertAfter + mod.code,
              );
              await fs.writeFile(filePath, content, "utf-8");
              console.log(
                `  Inserted code after "${mod.insertAfter.substring(0, 40)}..." in ${mod.file}`,
              );
            } else {
              console.warn(
                `  Warning: Could not find insertion point "${mod.insertAfter}" in ${mod.file}`,
              );
            }
          } catch (error: any) {
            console.error(
              `  Error inserting into ${mod.file}: ${error.message}`,
            );
          }
        }
        break;
...
`;
  ...
  for (const mod of evalConfig.modifications) {
    if (mod.type === "replace" && mod.file && mod.replace) {
      if (
        mod.replace.includes("wrong-selector") ||
        mod.replace.includes("user-name")
      ) {
        errorDetails += `
...
async function generateFixture(evalConfig: EvalConfig): Promise<void> {
  ...
  if (evalConfig.needsFailureLog) {
    ...
    await fs.writeFile(
      path.join(fixtureDir, "failure.log"),
      failureLogContent,
      "utf-8",
    );
  ...
  const fixtureMetadata = {
    ...
    modifications: evalConfig.modifications.map((m) => ({
      ...
      ...(m.insertAfter && {
        insertAfter: m.insertAfter.substring(0, 50) + "...",
      }),
...
async function main(): Promise<void> {
  ...
  console.log("\n" + "=".repeat(60));
  console.log(
    `Successfully generated ${evalsToGenerate.length} fixtures in ${duration}s`,
  );
  console.log("=".repeat(60));
  ...
  for (const [tier, ids] of Array.from(tiers.entries()).sort(
    (a, b) => a[0] - b[0],
  )) {
